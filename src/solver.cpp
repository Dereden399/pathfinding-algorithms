...
void AStarSolver::solve(Problem& problem) {
    ...
    std::vector<std::vector<int>> parent(problem.n,
                                         std::vector<int>(problem.m, -1));
    ...
    while (!queue.empty()) {
        ...
        for (auto [dx, dy] : directions) {
            ...
            if (cost[nx][ny] == -1 || newCost < cost[nx][ny]) {
                cost[nx][ny] = newCost;
                parent[nx][ny] = x * problem.n + y;
                queue.push({newCost, nx, ny});
            }
        }
    }
    // now need to build the path
    for (int x = problem.goal[0], y = problem.goal[1]; x != -1 && y != -1;
         x = parent[x][y] / problem.n, y = parent[x][y] % problem.n) {
